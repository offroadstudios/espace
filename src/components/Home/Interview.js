      ...
      >
        <div className="container mx-auto h-full">
          <div className="flex flex-col lg:flex-row justify-center h-full">
            ...
            <motion.div
              variants={fadeIn('up')}
              className={`h-[812px] lg:h-auto lg:w-1/2 ${isExpanded ? 'h-screen w-screen' : ''}`}
              onClick={() => setIsExpanded(!isExpanded)}
            >
              <img src={bgpart1} alt="Background Part 1" className="object-contain h-full w-full" />
            </motion.div>
            {/* Image part 2 */}
            <motion.div
              variants={fadeIn('up')}
              ...
              onClick={() => setIsExpanded(!isExpanded)}
            >
              <img src={bgpart2} alt="Background Part 2" className="object-contain h-full w-full" />
            </motion.div>
          </div>
        </div>
      </motion.section>
